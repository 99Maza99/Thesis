...
class Buttons : ##Basic buttons that work on their own
    def add(table,instances): 
        Commandtool.Insert.intotable(table, Commandtool.Select.allfromentry(instances,"entry"))
        Commandtool.Delete.clear(instances=instances)


    def search(conn,frame,instances,table):
        global searchdata
        global originaldata
        global counter    
...
class Autofillcycle:##Buttons that rely on each other to make the functionality of a search cycle
    def next(instances,table,frame):
        ...
        if table == "Personel":
            data = Commandtool.Organize.fromdatabase(buffer,table)
...
class Frames : ##All entry frames !
    def entryframe(root,table):
        ...
        priceentry   = Entry(entryframe, width=10, validate='key', validatecommand=(entryframe.register(partial(Commandtool.Window.onlynum)),'%S','%P'))
        ...
        pricelabel   = Label(entryframe, text="Price")


        ##Creating instances of buttons widgets
        instances     = [IDentry,productentry,quantityentry,priceentry]


        addbutton     = Button(entryframe,text="Add to bill"    ,command= lambda:Buttons.add(table=table,instances=instances))
        ...
        priceentry.grid   (row=1,column=3,sticky=W)
        ...
        addbutton.grid     (row=2,column=1,sticky=W)

        
        
        return entryframe,instances

    def personelframe(conn,root): 
        ...
        phonenumberentry = Entry(infoframe2,width = 60,validate='key' ,validatecommand=(infoframe2.register(partial(Commandtool.Window.onlynum, decimal = False)),'%S','%P'))
        ...
        Searchbutton = Button(infoframe2,text="Search",command= lambda:Buttons.search(conn,frame,entries,"Personel"))
...
class Table: ##Stores all the Tables that will be used in any GUI file upfront
    def billtable(billno=0,root=None): ##This is a Table that stores all the info of a bill before transferring it into the database !
